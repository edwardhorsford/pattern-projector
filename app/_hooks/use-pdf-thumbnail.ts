...
// Empty layers object - stable reference to avoid triggering re-renders
...
export function usePdfThumbnail(
  ...
  // Create a stable key for layers to avoid re-running effect when layers object
  // is recreated with the same content
  const layersKey = useMemo(
    () => JSON.stringify(Object.keys(layers).sort().map(k => [k, layers[k].visible])),
    [layers]
  );
  ...
  // Note: layersKey provides stable comparison for layers changes
